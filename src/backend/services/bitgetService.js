...
class BitgetService {
  ...
  async getAccountInfo() {
    return this.makeRequest('GET', '/api/v2/spot/account/assets');
  }

  // Get market data - endpoint aggiornato
  async getMarketData(symbol) {
    return this.makeRequest('GET', '/api/v2/spot/market/ticker', { symbol });
  }
  ...
  async placeOrder(symbol, side, orderType, price, quantity) {
    ...
    return this.makeRequest('POST', '/api/v2/spot/trade/place-order', {}, data);
  }

  // Cancel order - endpoint aggiornato
  async cancelOrder(symbol, orderId) {
    return this.makeRequest('POST', '/api/v2/spot/trade/cancel-order', {}, { symbol, orderId });
  }

  // Get order history - endpoint aggiornato
  async getOrderHistory(symbol) {
    return this.makeRequest('GET', '/api/v2/spot/trade/history-orders', { symbol });
  }

  // Get open orders - endpoint aggiornato
  async getOpenOrders(symbol) {
    return this.makeRequest('GET', '/api/v2/spot/trade/open-orders', { symbol });
  }
  ...
  async getKlines(symbol, period, startTime, endTime, limit = 100) {
    return this.makeRequest('GET', '/api/v2/spot/market/candles', {
      symbol,
  ...
  async testConnection() {
    try {
      ...
      const response = await axios.get(`${this.baseUrl}/api/v2/common/time`);
      return {
        success: true,
        data: response.data
      };
  ...
  async testAuthentication() {
    try {
      ...
      return await this.makeRequest('GET', '/api/v2/spot/account/info');
    } catch (error) {
